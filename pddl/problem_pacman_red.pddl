;;Scenario&Assumption: The map used is the default map. In each team, there is a offensive pacman that concentrates on eating dots, and a defensive ghost that never across the middle line. The pacman on the red side is not powered, the pacman intends to eat all the dots on the opposite side, avoiding the ghost at the same time, then return to home.

(define (problem pacman_red)
